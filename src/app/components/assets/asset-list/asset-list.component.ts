...
export class AssetListComponent implements OnInit, OnDestroy {
    ...
    async getCoins(assetList) {
        ...
        for (let list of assetList) {
            for (let item of this.coinsData) {
                if (item.symbol === "XTAR") this.xtarPrecision = item.decimal;
    ...
    deo_send(step?: String) {
        ...
        if (this.deo_pwd || step === 'step1') {
            this.load.loadingShow();
            setTimeout(() => {
                this.auxBt.deposit(option, this.deo_pwd).then((res: any) => {
                    if (res.error.code != 0) return this.load.tipErrorShow(res.error.message || '')
                    else this.deoInter(res);
                })
            }, 10)
        }
    }
        ...
        this.deo_time = setInterval(() => {
            this.auxBt.client.GetEventLog(res.result).then(res => {
                ...
                if (res.result && res.result.error_code === 0) {
                    this.setTokenBlanace();
                    clearInterval(this.deo_time);
                    return this.assetUpata(2);
                } else {
                    if (num > 8) {
                        this.load.hide();
                        clearInterval(this.deo_time);
                        this.load.tipSuccessShow(this.translate.instant('common.selectError'));
                    }
                }
            })
        }, 1000)
    ...
    /**
     * 资产列表更新
     * @param type  1 ==>  提币    2 ==> 充值
     */
    assetUpata(type) {
        const success = data => {
            if (data.status === 0) {
                this.deo_time = null;
                this.setExchangeBlanace(data.data);
                this.load.hide();
                if (type === 1) this.load.tipSuccessShow(this.translate.instant('common.withdrawSuccess')), this.sub_flag = false;
                else this.load.tipSuccessShow(this.translate.instant('common.depositSuccess')), this.add_flag = false, this.dep_flag = 'step1';
            } else {
                this.load.tipErrorShow(data.msg)
            }
        };
        (this.deo_time || type === 1) && this.service.getBalance({symbol: this.symbolInfo.symbol}).then(res => {
            success(res)
        })
    }
     ...
     * 查询并修改token余额
     */
    async setTokenBlanace() {
        await this.auxBt.getBlanace(this.contractAddress).then((res: any) => {
     ...
     * @param data
     */
    setExchangeBlanace(data) {
        const fIndex: any = this.assetList.findIndex((ele) => {
            return ele.symbol === this.symbolInfo.symbol;
        });
