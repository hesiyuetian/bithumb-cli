...
export class MemberComponent implements OnInit {
    ...
    async checkBlance() {
        // xtarInfo信息
        let xtarInfo = null, balance;
        await this.reset.getCoins().then((res: any) => {
            xtarInfo = res.filter(res => {
                return res.symbol === 'XTAR';
            })[0];
        });
        this.auxBt.getBlanace(xtarInfo.contract_address).then((res: any) => {
            balance = this.regular.toFixed(res.result / (10 ** xtarInfo.decimal) || 0, xtarInfo.decimal);
    ...
    checkPrime(){
        this.auxBt.HasFatherNameTag(this.user.userId()).then( (res:any) => {
            console.log('res',res, this.user.userId())
        })
    }
        ...
        this.deo_time = setInterval(() => {
            this.auxBt.client.GetEventLog(res).then(res => {
                ...
                if (res.result && res.result.error_code === 0) {
                    clearInterval(this.deo_time);
                    this.load.tipSuccessShow(this.translate.instant('common.regiterSuccess'));
                    return this.router.navigateByUrl('/assets_records/list');
                } else {
                    if (num > 8) {
                        this.load.hide();
                        clearInterval(this.deo_time);
                        this.load.tipSuccessShow(this.translate.instant('common.selectRegiterError'));
                    }
                }
            })
        }, 1000)
